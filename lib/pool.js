...
const profitsOfPool = async ({ eth, pool, address }) => {
  ...
  // const profits = await contract.profitsOfPool(address).call() // use this if the pool is a v1 pool
  const contractAddress = POOL_CONTRACT_ADDRESSES[pool]
...
const infoOfPool = async ({ eth, pool, address }) => {
  const { contract, _ } = DashboardContract({ eth })
  // const info = await contract.infoOfPool(address).call() // use this in case the pool is a v1 pool
  const contractAddress = POOL_CONTRACT_ADDRESSES[pool]
